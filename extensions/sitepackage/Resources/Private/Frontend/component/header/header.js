...
Tilda.Component.Header = (() => {
    ...
    self.init = () => {
        $(window).on('changed.zf.mediaquery scroll load', false, function (evt) {
            if (Tilda.Function.getOffset() > self.settings.offset && !_.isScrolled) {
                _.isScrolled = true
                self.addState(self.classes.fixed)
            } else if (Tilda.Function.getOffset() <= self.settings.offset && _.isScrolled) {
                _.isScrolled = false
                self.delState(self.classes.fixed)
            }
        })

        if (self.elements.wrapper && !self.elements.wrapper.querySelector(self.selectors.stage)) {
