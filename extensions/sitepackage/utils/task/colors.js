...
const buildColors = (done) => {
    function render () {
        let markup = ''
        const fsRead = fs.readFileSync(pathConf.colors.src, { encoding: 'utf8', flag: 'r' })
        const regex = /\/\*\s?@color\s?\*\/\s?([\n\r\s\-A-Za-z0-9()$#':;, ]+)\s?\/\*\s?\/@color\s?\*\//gm
    ...
    return file(pathConf.colors.create, render(), { src: true })
        .pipe(dest((file) => file.base))
}

const watchColors = (done) => {
    return watch(pathConf.colors.src,
        {},
